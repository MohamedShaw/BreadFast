...
import { AppView, AppIcon, AppText, AppSpinner } from '.';
...
class Input extends PureComponent {
  ...
  getAsyncValidationScheme = value => {
    if (!value) return;
    if (this.state.asyncValidateLoading && this.source) {
  ...
  onChangeText = (text, noValidate) => {
    ...
    if (onChange) {
      if (name) onChange(name, text, !this.state.isTouched);
      else onChange(text);
    }
